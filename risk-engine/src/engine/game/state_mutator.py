...
class StateMutator():
    ...
    def commit(self, record: RecordType):
        ...
        match record:
            ...
            case RecordShuffledCards() as r:
                self._record_shuffled_cards(r)
            case RecordStartGame() as r:
                self._record_start_game(r)
            case RecordStartTurn() as r:
                self._record_start_turn(r)
            case RecordTerritoryConquered() as r:
                self._record_territory_conquered(r)
            case RecordWinner() as r:
                self._record_winner(r)
            case _:
    ...
    def _record_shuffled_cards(self, r: RecordShuffledCards) -> None:
    ...
    def _record_start_game(self, r: RecordStartGame) -> None:
        self.state.turn_order = list(r.turn_order).copy()


    def _record_start_turn(self, r: RecordStartTurn) -> None:
        self.state.players[r.player].troops_remaining += r.territory_bonus + r.continent_bonus


    def _record_territory_conquered(self, r: RecordTerritoryConquered) -> None:
    ...
    def _record_winner(self, r: RecordWinner) -> None:
        pass
