...
class ResponseAttackTerritory(BaseModel):
    ...
    @classmethod
    def _check_territory_attackable(cls, v, info: ValidationInfo):
        state = info.context["state"] # type: ignore
        player = info.context["player"] # type: ignore

        source_territory_id = v["source_territory_id"]
        target_territory_id = v["target_territory_id"]
        attacking_troops = v["attacking_troops"]
